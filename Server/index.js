    ...
    function addMarker(lat, long, username){
        var marker;
    ...
    function loadLocations(){
      ...
      .on('success', function(data){
        ...
        for (var i = 0; i < locations.length; i++) {
            ...
            if(found < 0){
              addMarker(locations[i].lat, locations[i].long, locations[i].username);
              map.fitBounds(bounds);
            }
            else
              markers[found].marker.setPosition(new google.maps.LatLng(locations[i].lat, locations[i].long))
        }
      })
      .go();
    }
